...
- [x] 1. Add plan configuration module and config fields
...
- [x] 2. Extend the data layer for subscriptions and payments
- [x] 2.1 Add user subscription persistence to both DB backends
...
- [x] 2.2 Add payments persistence to both DB backends
...
- [x] 2.3 Unit-test the DB layer with the in-memory backend
  - Test default backfill on upsert, plan counts, pending lookup returns only open statuses, payment update transitions.
  - _Requirements: 2.1, 2.3, 2.4_

- [x] 3. Implement SubscriptionService (lazy reset + expiry + enforcement)
- [x] 3.1 Create `bot/subscription.py` with `get_state` and `can_generate`
...
- [x] 3.2 Add plan mutation + analytics methods
...
- [x] 3.3 Unit-test SubscriptionService
...
- [x] 4. Add per-plan link expiry to tokens and the stream endpoint
- [x] 4.1 Extend token signing/verification in `bot/utils.py`
...
- [x] 6. Implement PaymentService
- [x] 6.1 Create `bot/payments.py` with purchase + UTR + decisions
...
- [x] 6.2 Unit-test PaymentService
...
- [x] 7. Build user-facing menus and commands in `bot/handlers.py`
- [x] 7.1 Enhanced `/start` menu with plan + usage
...
- [x] 7.2 Premium Plans page (`/plans` + callback) and `/myplan`
...
- [x] 8. Wire enforcement into file handling
...
- [x] 9. Implement purchase + UTR + admin-post flow
- [x] 9.1 Buy/Pay callbacks
...
- [x] 9.2 UTR text capture and admin-group posting
...
- [x] 10. Implement admin verification and management
- [x] 10.1 Approve/Reject callbacks (admin-only, idempotent)
...
- [x] 12. Integration tests (in-memory backend)
